...
async function getHomePageIdFromSlug(): Promise<string | null> {
  const fromHomeSlug = await fetchPageIdBySlug('(home)');
  if (fromHomeSlug) return fromHomeSlug;
  return fetchPageIdBySlug('home');
}
...
async function getData() {
  try {
    ...
    const homePageIdFromSlug = settings.homePageId ? null : await getHomePageIdFromSlug();
...
export default async function Home() {
  const { settings, posts, homePageIdFromSlug } = await getData();
  const homePageId = settings.homePageId || homePageIdFromSlug;
  
  // If a custom homepage is selected (or a home slug exists), render that page instead.
  if (homePageId) {
    return <PageByIdRenderer pageId={homePageId} />;
  }
